...
public class UserDataDAO {
    ...
    public ArrayList search(UserDataDTO ud) throws SQLException{
        ...
        try{
            ...
            boolean flgName = false;
            boolean flgBirthday = false;
            boolean flgType = false;
            ...
            //SQL文のインジェクション分岐。flgCntにてインジェクション位置を調整。
            int flgCnt = 0;
            
            if (flgName){
                flgCnt ++;
                st.setString(flgCnt, "%"+ud.getName()+"%");
            }
            if (flgBirthday){
                flgCnt ++;
                st.setString(flgCnt, "%"+ new SimpleDateFormat("yyyy").format(ud.getBirthday())+"%");
            }
            if (flgType){
                flgCnt ++;
                st.setInt(flgCnt, ud.getType());
            }
