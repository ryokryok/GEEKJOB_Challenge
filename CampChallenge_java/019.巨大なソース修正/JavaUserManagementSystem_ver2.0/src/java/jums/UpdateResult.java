...
public class UpdateResult extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //更新するデータのIDをセッションに保存していたデータから取得
            UserDataDTO get4ID = (UserDataDTO)session.getAttribute("resultData");
            update.setUserID(get4ID.getUserID());
            
            //アップデート情報をDB側に反映させる
            UserDataDAO.getInstance().update(update);
            
            UserDataDTO resultData = new UserDataDTO();
            resultData = UserDataDAO.getInstance().searchByID(update);
            
            //変更結果に表示させるためにsession登録
