 ...
 * @author hayashi-s
 * 2017/10/16 add method that change UserDataDTO to UserDataBeans. by Ryo Kato.
 */
public class UserDataBeans implements Serializable{
    ...
    public UserDataBeans DTO2UDB(UserDataDTO udd){
        //Date型データを年月日に分割するための準備
        Calendar birthday = Calendar.getInstance();
        birthday.setTime(udd.getBirthday());
        
        UserDataBeans udb = new UserDataBeans();
        udb.setName(udd.getName());
        udb.setYear(String.valueOf(birthday.get(Calendar.YEAR)));
        udb.setMonth(String.valueOf(birthday.get(Calendar.MONTH) + 1));
        udb.setDay(String.valueOf(birthday.get(Calendar.DAY_OF_MONTH)));
